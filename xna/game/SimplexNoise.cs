...
namespace Garden
{
    // This code based on public domain code from Stefan Gustavson
    // (stegu@itn.liu.se) and Peter Eastman (peastman@drizzle.stanford.edu).
    class SimplexNoise
    {
        static Vector3[] grad3 = new Vector3[]{
        ...
        static float F2 = (float)(0.5 * (Math.Sqrt(3.0) - 1.0));
        static float G2 = (float)((3.0 - Math.Sqrt(3.0)) / 6.0);
        ...
        static float Dot(Vector3 g, float x, float y)
        {
            return g.X * x + g.Y * y;
        }
    }
